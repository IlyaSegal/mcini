...
void convertDCMQGSM_SMM (TString inFile  = "CAS-SMM-evt.out", TString outFile = "test.root", int nEvents = 5000, int splitFactor = 1)
{
...
//    cout << nEvent << endl;
    if (nEvent % eventsPerFile == 0)
    {
      if (tree) tree -> Write ();
      if (f) f -> Close ();
      f = new TFile (outFile.Data (), "recreate");
    ...
    skipFlag = false;
    event -> SetParameters (eventNr, b, phi, nes, stepNr, stepT, bx, by);
    iniState -> setId(eventNr);
    int index = 0;
    
    for (int nucleonIndex = 0; nucleonIndex < aProj; nucleonIndex++)
    {
      double charge1, x1 = 0, y1 = 0, z1 = 0, t1 = 0, charge2, x2 = 0, y2 = 0, z2 = 0, t2 = 0, px1 = 0, py1 = 0, pz1 = 0, m1, px2 = 0, py2 = 0, pz2 = 0, m2;
      *InputFile >> x1 >> y1 >> z1 >> charge1 >> x2 >> y2 >> z2 >> charge2;
//      cout << x1 << "\t" << y1 << "\t" << z1 << "\t" << charge1 << "\t" << x2 << "\t" << y2 << "\t" << z2 << "\t" << charge2 << endl;
      ...
      TLorentzVector position1(x1, y1, z1, t1);
      TLorentzVector position2(x2, y2, z2, t2);
      TLorentzVector momentum1, momentum2;
      momentum1.SetXYZM(px1, py1, pz1, m1);
      momentum2.SetXYZM(px2, py2, pz2, m2);
      Nucleon nucleon1, nucleon2;
      nucleon1.setId(nucleonIndex);
      nucleon1.setCharge(charge1);
      nucleon1.setPdgId(getPdgType(1, charge1, 0));
      nucleon1.setPosition(position1);
      nucleon1.setMomentum(momentum1);
      iniState->addNucleon(nucleon1);
      nucleon2.setId(nucleonIndex+header->GetAProj());
      nucleon2.setCharge(charge2);
      nucleon2.setPdgId(getPdgType(1, charge2, 0));
      nucleon2.setPosition(position2);
      nucleon2.setMomentum(momentum2);
      iniState->addNucleon(nucleon2);
    }
